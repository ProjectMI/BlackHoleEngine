...
int main()
{
    ...
    engine.AddObject(std::make_unique<StarSkybox>(2000, engine.Width, engine.Height));
    engine.AddObject(std::make_unique<BlackHole>(4.f));
    engine.GetObject(1)->SetPosition(Vector3D(10, 5, 5));
    engine.AddObject(std::make_unique<BlackHoleLensEffect>(dynamic_cast<BlackHole*>(engine.GetObject(1)), dynamic_cast<StarSkybox*>(engine.GetObject(0)), engine.GetCamera()));
